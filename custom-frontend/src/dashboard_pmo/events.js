...
function toggleModel(action,event){
  ...
  setDisplayData({...event, convertDate:date.getFullYear() + "/" + (date.getMonth() + 1) + "/" + date.getDate()});
  if(action === "update"){
    ...
    <>
      <div className='data-container'>
        <div className='data-container-top'>
          ...
          <button className='add-data-btn' onClick={() => toggleModel("add",false)}>
            Add New Event
          </button>
        </div>
        {!getData ? (
          ...
          <table className='table'>
            ...
            <tbody>
              ...
              {getData.filter((event) =>(event[searchValue].toString().indexOf(searchData) > -1)).slice(0).reverse().map((event) => {
                let date = new Date(event.date);
                return (
                  <tr key={event._id}>
